...
## Starwake request loop (every feature)

Not just throttle. **Every** update and new feature — flight, crew, market, ranks, mining, HUD, 3D — runs this before the first patch:

1. **Comps.** Name 2–4 games that already solved it (Elite, Star Citizen, X4, NMS, KSP, Freelancer, Everspace, as relevant).
2. **Steal / don't.** One line: Starwake is a Helion **trader**, not a sim. Write what we take and what we refuse.
3. **Quality bar.** A test table or Decision Log row *with* the patch (see Decision #016). Numbers that fail if the feel regresses.
4. **Then implement.** If the first pass is jumpy or off-genre, the bar was too thin — tighten it, don't guess twice.

Do not skip this for "small" asks. Research in the reply; the bar in the repo.
